//! 다시 살펴보기 강사님 깃허브 !!!!!!! 작동 안됨!!!!!!!!!

//! 1. 태그 이름과 클래스 이름 리스트를 인수로 받아
//! 새로운 HTML 요소를 생성하고 반환하는 create 함수

// div 태그에 class명이 container, special인 요소 생성 시
// : create('div', ['container', 'special']) 함수 호출
//? : id 매개변수의 경우 해당 프로젝트에서 사용하지 않는다.
...
form.addEventListener('submit', (event) => {
  ...
  // : 웹 페이지의 로컬 스토리지에 기존에 저장된 할 일 목록(배열)이 존재한다면
  //   taskArr 배열에 저장
  ...
  // : alreadIncluded 배열은 taskArr에서 task의 값만을 가져온 문자열 배열
  let alreadyIncluded = [];
...
toDoList.addEventListener('click', (e) => {
  ...
  if (dlt.includes(e.target)) {
    ...
    for (let i = 0; i < list.length; i++) {
      // : 클릭된 삭제 버튼의 부모 요소인 할 일 항목의 텍스트가 
      //   현재 순회 중인 할 일과 일치하는 지 확인
      ...
      if (list[i].task.replace(/[\n\r]+|[\s]{2,}/g, ' ').trim() === e.target.parentNode.innerText.replace(/[\n\r]+|[\s]{2,}/g, ' ').trim()) {
        ...
        localStorage.setItem('Tasks', JSON.stringify(list));
        // toDoList 요소의 내부 HTML을 초기화
        // : 삭제된 할 일이 포함된 HTML을 제거
        toDoList.innerHTML = '';

        initializeTaskList(); 
        // 할 일 목록을 재초기화 - 삭제된 할 일은 출력하지 않음
        break;
      }
    }
  }
