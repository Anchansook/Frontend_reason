...
class Membership<T extends Member> {
  ...
  // : active 속성이 true인 회원들만 새로운 배열로 만들어 반환
...
// 회원관리 로직이 하나 생성(일반 회원 & VIP 회원 모두를 관리)
let membership = new Membership<Member>();

//? 회원을 추가하는 예시
let memberA = new Member('이용주', 28);
membership.addMember(memberA);
membership.addMember(new Member('정재형', 30, false));
membership.addMember(new Member('김민수', 26));
membership.addMember(new Member('안찬숙', 22, false));

//? VIP 멤버를 회원 목록에 추가
membership.addMember(new VIPMember('전지현', 40, 500, false));
membership.addMember(new VIPMember('김수현', 30, 100));
membership.addMember(new VIPMember('송혜교', 20, 50, false));

//? '이용주' / '김재형' 을 검색 및 출력하는 예시
console.log(membership.findMember('이용주'));
// Member { name: '이용주', age: 28, active: true }
console.log(membership.findMember('김재형'));
// undefined
...
[
  Member { name: '이용주', age: 28, active: true },
  Member { name: '김민수', age: 26, active: true }
]
*/

//? '안찬숙' 회원 제거 및 검증 예시
membership.removeMember('안찬숙');
console.log(membership.findMember('안찬숙')); // undefined - 삭제 완

//? 모든 회원의 평균 나이 계산 및 출력 예시
console.log(membership.getAverageAge()); // 29
...
/*
[
  Member { name: '정재형', age: 30, active: false },
  VIPMember { name: '전지현', age: 40, active: false, rewardPoints: 500 },
  VIPMember { name: '송혜교', age: 20, active: false, rewardPoints: 50 }
]
*/

// VIP 회원들만 따로 관리하는 회원 목록
let vipMembership = new Membership<VIPMember>();
vipMembership.addMember(new VIPMember('안찬숙', 29, 1000, true));
vipMembership.addMember(new VIPMember('정수경', 30, 500, false));
vipMembership.addMember(new VIPMember('백지희', 28, 700, true));
vipMembership.addMember(new VIPMember('김지원', 27, 100, false));

console.log(vipMembership.getAverageAge()); // 28.5
console.log(vipMembership.filterActiveMembers());
/*
[
  VIPMember { name: '안찬숙', age: 29, active: true, rewardPoints: 1000 },
  VIPMember { name: '백지희', age: 28, active: true, rewardPoints: 700 }
]
*/
console.log(vipMembership.filterInactiveMembers());
/*
[
  VIPMember { name: '정수경', age: 30, active: false, rewardPoints: 500 },
  VIPMember { name: '김지원', age: 27, active: false, rewardPoints: 100 }
]
*/
