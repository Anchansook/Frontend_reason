...
// : 기존 타입을 변환하거나 조작하는 데 사용

// 1) Partial: 모든 속성을 선택적으로 만들어 줌
// 형태
// : Partial<T>
interface User {
  name: string;
...
// {name: '이용주', age: 28, name: 'ASC'}
// >> {name: 'ASC', age: 28}
updateUser(1, {name: 'ASC'});
updateUser(2, {age: 29});
updateUser(3, {name: '미상', age: 50}); // User with id 3 not found

console.log(users[1]); // { name: 'ASC', age: 28 }
console.log(users[2]); // { name: '정재형', age: 29 }
console.log(users[3]); // 새로운 객체의 값이 지정 >> { name: '미상', age: 50 }
