...
const initialGoals :Goal[] = [
  ...
  },
  {
    id: 2,
    title: '복습하기',
    explanation: '수업 진도 복습하기',
    active: false
  },
  {
    id: 3,
    title: '공부하기',
    explanation: '자격증 공부하기',
    active: false
  }
]
...
export default function GoalApp() {
  ...
  const handleChange = (e :ChangeEvent<HTMLInputElement>) => {
    ...
    //   변경된 name의 value값만 '상태 설정 함수'로 업데이트
    setGoalInput({
      ...goalInput,
      [name]: value
    });
  ...
  const hendleToggle = (id :number) => {
    // 전체 배열 순회(map) goal.id가 매개변수와 일치하는 요소는
    // 전체 속성 중에서 active 값만 반전
    // : 일치하지 않는 요소는 그대로 반환
