...
// 2. 입력 유효성 검사: 모든 필드는 비어져 있을 수 없음
//    , 이메일은 유효한 형식을 지정
...
export default function State02() {
  ...
  const validateEmail = (email: string): boolean => {
    //? test() 메서드
    // : 주어진 문자열이 정규 표현식을 만족하는 지 판별
    //   그 여부를 boolean으로 반환
    // 만족할 경우 true, 만족하지 않을 경우 false
    return /\S+@\S+\.\S+/.test(email);

    /*
    ? 정규 표현식(이메일: ckstnr88@naver.com)
      : \S+@\S+\.\S+
      \S+ : 하나 이상의 공백이 아닌 문자에 일치(포함)(아이디) (ckstnr88)
      @ : 리터럴 문자인 @ 문자를 포함 (@)
      \S+ : 하나 이상의 공백이 아닌 문자에 일치(포함)(도메인 이름의 시작)
      \. : 리터럴 점(.)문자를 포함(도메인의 일부)
      \S+ : 하나 이상의 공백이 아닌 문자에 일치(포함)
