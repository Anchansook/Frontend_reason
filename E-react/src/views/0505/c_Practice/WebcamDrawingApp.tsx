...
export default function WebcamDrawingApp() {
  ...
  //! 사용자의 변경 상태를 저장하는 useState Hook 사용
  // 1) 그리기 상태를 관리
  ...
  const startDrawing = useCallback((event :React.MouseEvent<HTMLCanvasElement>) => {
    ...
    //& current?
    // : 옵셔널 체이닝 - 해당 값이 null 또는 undefined 인 경우 
    //   뒤의 메소드를 호출하지 않고 undefined를 반환
  ...
  const draw = useCallback((event :React.MouseEvent<HTMLCanvasElement>) => {
    ...
    if (context) {
      ...
      context.strokeStyle = drawingMode ? brushColor : '#ffffff' // 선의 색상 설정
