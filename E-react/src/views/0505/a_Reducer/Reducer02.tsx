...
const gameReducer = (state :GameState, action :Action) :GameState => {
  switch (action.type) {
    ...
    // 'USE_POTION' 액션 처리: 플레이어가 보유한 표션 수가 0보다 크면 포션 수를 1 감소
    case 'USE_POTION' :
...
export default function Reducer02() {
  ...
  return (
    <div>
      ...
      <p>Potions: {state.player.item.potion}</p>
      <button onClick={() => dispatch({ type: 'LEVEL_UP' })}>Level Up</button>
      <button 
        onClick={() => dispatch({ type: 'USE_POTION' })}
        disabled={state.player.item.potion === 0}
      >
          Use Potion ({state.player.item.potion})
      </button>
    </div>
  )
}
