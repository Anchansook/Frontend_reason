...
// : 한 작업이 완료될 때까지 다음 작업은 대기(시작하지 않음)
...
function performCalulation(): number {
  // 시간 지연을 위한 계산식
  let sum = 0;
...
function performAnotherCalulation(): number {
  // 시간 지연을 위한 계산식
  let product = 1;
  for (let i = 1; i < 100; i++) {
    product *= i;
...
function syncFunction() {
  ...
  let result = performCalulation();
  console.log('첫 번째 작업 완료, 결과: ', result);

  console.log('두 번째 작업 시작');
  let anotherResult = performAnotherCalulation();
