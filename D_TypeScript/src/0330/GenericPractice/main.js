...
var TaskManager = /** @class */ (function () {
    ...
    TaskManager.prototype.renderTasks = function (containerId) {
        ...
        this.tasks.forEach(function (task) {
            ...
            // JS로 생성된 DOM 요소를 HTML에서 추가하기 위해서는
            // : 생성되어 있는 HTML의 하위 요소로 전달(appendChild)
            // - 삭제 버튼을 목록 항목에 추가
...
window.onload = function () {
    ...
    addButton.onclick = function () {
        ...
        if (newTaskInput.value.trim() !== '') {
            ...
            // 할 일을 추가한 후, 입력 필드 초기화(즉, input창)
            newTaskInput.value = '';
        }
    };
};
// html파일에 ts코드를 연결하기 위해서는
// : tsc(typescript compiler)를 사용하여 js파일로 컴파일한 후 연결
